...
sheet_comp <- function(t1, t2, digits.signif = 4, proportional.diff = FALSE) {
  ...
  mat.new <- mat1
  mat.new[ind.diff.text] <- paste0(
    mat1[ind.diff.text], " --> ",
    mat2[ind.diff.text]
  )
  if (proportional.diff) {
    mat.new[ind.diff.num] <- paste0(
      signif(as.numeric(mat1[ind.diff.num]), digits.signif + 2), " --> ",
      signif(as.numeric(mat2[ind.diff.num]), digits.signif + 2)
    )
    mat.new[ind.diff.num.percents] <- suppressWarnings(
      paste0(
        signif(
          as.numeric(gsub("%$", "", mat1)[ind.diff.num.percents]),
          digits.signif + 2
        ), "% --> ",
        signif(as.numeric(as.numeric(gsub("%$", "", mat2))[ind.diff.num.percents]), digits.signif + 2), "%"
      )
    )
  } else {
    mat.new[ind.diff.num] <- paste0(
      signif(as.numeric(mat1[ind.diff.num]), digits.signif), " --> ",
